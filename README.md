...
### Инструменты ИИ в разработке (opencode + MCP)

Проект разработан с использованием AI-агента как «парного программиста». Архитектурные решения (послойная структура, Dynamic Overlay, трёхслойная система токенов) приняты разработчиком; ИИ ускорял реализацию и контроль качества. Рабочие конфиги агента (`.opencode/`, `.project/`, `.review/`) в репозиторий не входят — ниже описан процесс.

- **opencode (AI-агент)** — генерация и рефакторинг кода по задачам; семантический индекс кодовой базы для навигации; LSP-диагностика TS/SCSS в процессе правок.
- **Локальная база знаний и skills** — документация Angular (llms.txt) и переиспользуемые инструкции-«скиллы» (иконки, локации элементов, анимации формы поиска): агент сверялся с актуальными API фреймворка вместо угадывания, поэтому паттерны Angular 21 (signals, `model`, `linkedSignal`, `@let`, control flow) применяются последовательно.
- **MCP Figma** (figma-developer-mcp) — данные макета (геометрия, цвета, иконки) забирались напрямую из Figma: размерная шкала дизайн-токенов и SVG в `assets/icons` — из макета, без ручного перемера.
- **MCP Playwright** — автоматическая проверка в живом браузере: computed-стили, геометрия, поведение на mobile/desktop. Например, фиксы padding'ов кнопок `.action` на mobile и justify-content в модалке проверялись по живому DOM, а не «на глаз».
- **Стандарты проекта как контекст для ИИ** — правила SCSS-архитектуры (слои токенов, нейминг переменных, контракты миксинов, стилизация компонентов) были зафиксированы в инструкциях до написания кода; агент был обязан им следовать. Это позволило удержать единую токенизированную систему во всех стилях проекта.
- **AI-ревью кода** — агент периодически прогонялся с проверкой по этим инструкциям: находки (мёртвый код, незакрытые `@property`, сломанные анимации, хардкод значений при живых токенах) триажировались и исправлялись — следы видны в истории коммитов.
- **Общий гейт качества** — вывод ИИ проходит тот же контроль, что и человеческий код: ESLint + Prettier, git-хуки (pre-commit / pre-push), `ng build` с бюджетами.
